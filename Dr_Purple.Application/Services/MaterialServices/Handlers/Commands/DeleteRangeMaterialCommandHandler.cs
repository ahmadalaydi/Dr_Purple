...
public class DeleteRangeMaterialCommandHandler : BaseMaterialCommandHandler,
    IRequestHandler<DeleteRangeMaterialCommand, IDataResult<MaterialResponse>>
{
    public DeleteRangeMaterialCommandHandler(IServiceProvider provider) : base(provider) { }

    public async Task<IDataResult<MaterialResponse>> Handle(DeleteRangeMaterialCommand? command, CancellationToken cancellationToken)
    {
        await Task.WhenAll();

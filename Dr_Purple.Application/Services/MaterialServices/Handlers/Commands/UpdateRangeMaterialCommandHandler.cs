...
public class UpdateRangeMaterialCommandHandler : BaseMaterialCommandHandler,
    IRequestHandler<UpdateRangeMaterialCommand, IDataResult<MaterialResponse>>
{
    public UpdateRangeMaterialCommandHandler(IServiceProvider provider) : base(provider) { }

    public async Task<IDataResult<MaterialResponse>> Handle(UpdateRangeMaterialCommand? command, CancellationToken cancellationToken)
    {
        await Task.WhenAll();

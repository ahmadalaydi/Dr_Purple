...
public class CreateMaterialCommandHandler : BaseMaterialCommandHandler,
    IRequestHandler<CreateMaterialCommand, IDataResult<MaterialResponse>>
{
    public CreateMaterialCommandHandler(IServiceProvider provider) : base(provider) { }

    public async Task<IDataResult<MaterialResponse>> Handle(CreateMaterialCommand? command, CancellationToken cancellationToken)
    {
        await Task.WhenAll();

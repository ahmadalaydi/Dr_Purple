...
public class DeleteMaterialCommandHandler : BaseMaterialCommandHandler,
    IRequestHandler<DeleteMaterialCommand, IDataResult<MaterialResponse>>
{
    public DeleteMaterialCommandHandler(IServiceProvider provider) : base(provider) { }

    public async Task<IDataResult<MaterialResponse>> Handle(DeleteMaterialCommand? command, CancellationToken cancellationToken)
    {
        await Task.WhenAll();

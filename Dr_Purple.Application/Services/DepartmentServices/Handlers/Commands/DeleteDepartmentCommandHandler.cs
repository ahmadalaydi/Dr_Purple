...
public class DeleteDepartmentCommandHandler : BaseDepartmentCommandHandler,
    IRequestHandler<DeleteDepartmentCommand, IDataResult<DepartmentResponse>>
{
    public DeleteDepartmentCommandHandler(IServiceProvider provider) : base(provider) { }

    public async Task<IDataResult<DepartmentResponse>> Handle(DeleteDepartmentCommand? command, CancellationToken cancellationToken)
    {
        await Task.WhenAll();

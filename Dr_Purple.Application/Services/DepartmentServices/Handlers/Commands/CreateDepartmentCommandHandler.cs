...
public class CreateDepartmentCommandHandler : BaseDepartmentCommandHandler,
    IRequestHandler<CreateDepartmentCommand, IDataResult<DepartmentResponse>>
{
    public CreateDepartmentCommandHandler(IServiceProvider provider) : base(provider) { }

    public async Task<IDataResult<DepartmentResponse>> Handle(CreateDepartmentCommand? command, CancellationToken cancellationToken)
    {
        await Task.WhenAll();

...
public class UpdateDepartmentCommandHandler : BaseDepartmentCommandHandler,
    IRequestHandler<UpdateDepartmentCommand, IDataResult<DepartmentResponse>>
{
    public UpdateDepartmentCommandHandler(IServiceProvider provider) : base(provider) { }

    public async Task<IDataResult<DepartmentResponse>> Handle(UpdateDepartmentCommand? command, CancellationToken cancellationToken)
    {
        await Task.WhenAll();

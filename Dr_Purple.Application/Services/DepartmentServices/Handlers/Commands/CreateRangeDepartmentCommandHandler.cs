...
public class CreateRangeDepartmentCommandHandler : BaseDepartmentCommandHandler,
    IRequestHandler<CreateRangeDepartmentCommand, IDataResult<DepartmentResponse>>
{
    public CreateRangeDepartmentCommandHandler(IServiceProvider provider) : base(provider) { }

    public async Task<IDataResult<DepartmentResponse>> Handle(CreateRangeDepartmentCommand? command, CancellationToken cancellationToken)
    {
        await Task.WhenAll();

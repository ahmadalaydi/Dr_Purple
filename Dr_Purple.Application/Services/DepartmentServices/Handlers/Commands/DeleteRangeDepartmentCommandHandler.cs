...
public class DeleteRangeDepartmentCommandHandler : BaseDepartmentCommandHandler,
    IRequestHandler<DeleteRangeDepartmentCommand, IDataResult<DepartmentResponse>>
{
    public DeleteRangeDepartmentCommandHandler(IServiceProvider provider) : base(provider) { }

    public async Task<IDataResult<DepartmentResponse>> Handle(DeleteRangeDepartmentCommand? command, CancellationToken cancellationToken)
    {
        await Task.WhenAll();

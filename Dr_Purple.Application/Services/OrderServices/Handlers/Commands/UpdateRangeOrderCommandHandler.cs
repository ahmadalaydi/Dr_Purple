...
public class UpdateRangeOrderCommandHandler : BaseOrderCommandHandler,
    IRequestHandler<UpdateRangeOrderCommand, IDataResult<OrderResponse>>
{
    public UpdateRangeOrderCommandHandler(IServiceProvider provider) : base(provider) { }

    public async Task<IDataResult<OrderResponse>> Handle(UpdateRangeOrderCommand? command, CancellationToken cancellationToken)
    {
        await Task.WhenAll();

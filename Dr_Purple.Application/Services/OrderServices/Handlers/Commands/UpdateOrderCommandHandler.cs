...
public class UpdateOrderCommandHandler : BaseOrderCommandHandler,
    IRequestHandler<UpdateOrderCommand, IDataResult<OrderResponse>>
{
    public UpdateOrderCommandHandler(IServiceProvider provider) : base(provider) { }

    public async Task<IDataResult<OrderResponse>> Handle(UpdateOrderCommand? command, CancellationToken cancellationToken)
    {
        await Task.WhenAll();

...
public class DeleteOrderCommandHandler : BaseOrderCommandHandler,
    IRequestHandler<DeleteOrderCommand, IDataResult<OrderResponse>>
{
    public DeleteOrderCommandHandler(IServiceProvider provider) : base(provider) { }

    public async Task<IDataResult<OrderResponse>> Handle(DeleteOrderCommand? command, CancellationToken cancellationToken)
    {
        await Task.WhenAll();

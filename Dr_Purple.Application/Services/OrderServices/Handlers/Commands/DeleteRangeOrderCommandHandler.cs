...
public class DeleteRangeOrderCommandHandler : BaseOrderCommandHandler,
    IRequestHandler<DeleteRangeOrderCommand, IDataResult<OrderResponse>>
{
    public DeleteRangeOrderCommandHandler(IServiceProvider provider) : base(provider) { }

    public async Task<IDataResult<OrderResponse>> Handle(DeleteRangeOrderCommand? command, CancellationToken cancellationToken)
    {
        await Task.WhenAll();

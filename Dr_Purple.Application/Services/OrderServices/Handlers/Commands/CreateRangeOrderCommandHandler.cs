...
public class CreateRangeOrderCommandHandler : BaseOrderCommandHandler,
    IRequestHandler<CreateRangeOrderCommand, IDataResult<OrderResponse>>
{
    public CreateRangeOrderCommandHandler(IServiceProvider provider) : base(provider) { }

    public async Task<IDataResult<OrderResponse>> Handle(CreateRangeOrderCommand? command, CancellationToken cancellationToken)
    {
        await Task.WhenAll();

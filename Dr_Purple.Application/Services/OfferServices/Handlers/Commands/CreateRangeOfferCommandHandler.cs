...
public class CreateRangeOfferCommandHandler : BaseOfferCommandHandler,
    IRequestHandler<CreateRangeOfferCommand, IDataResult<OfferResponse>>
{
    public CreateRangeOfferCommandHandler(IServiceProvider provider) : base(provider) { }

    public async Task<IDataResult<OfferResponse>> Handle(CreateRangeOfferCommand? command, CancellationToken cancellationToken)
    {
        await Task.WhenAll();

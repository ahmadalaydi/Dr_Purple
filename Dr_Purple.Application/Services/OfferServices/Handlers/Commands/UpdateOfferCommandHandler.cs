...
public class UpdateOfferCommandHandler : BaseOfferCommandHandler,
    IRequestHandler<UpdateOfferCommand, IDataResult<OfferResponse>>
{
    public UpdateOfferCommandHandler(IServiceProvider provider) : base(provider) { }

    public async Task<IDataResult<OfferResponse>> Handle(UpdateOfferCommand? command, CancellationToken cancellationToken)
    {
        await Task.WhenAll();

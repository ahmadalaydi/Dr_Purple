...
public class CreateOfferCommandHandler : BaseOfferCommandHandler,
    IRequestHandler<CreateOfferCommand, IDataResult<OfferResponse>>
{
    public CreateOfferCommandHandler(IServiceProvider provider) : base(provider) { }

    public async Task<IDataResult<OfferResponse>> Handle(CreateOfferCommand? command, CancellationToken cancellationToken)
    {
        await Task.WhenAll();

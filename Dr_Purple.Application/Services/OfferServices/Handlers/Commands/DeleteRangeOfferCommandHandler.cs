...
public class DeleteRangeOfferCommandHandler : BaseOfferCommandHandler,
    IRequestHandler<DeleteRangeOfferCommand, IDataResult<OfferResponse>>
{
    public DeleteRangeOfferCommandHandler(IServiceProvider provider) : base(provider) { }

    public async Task<IDataResult<OfferResponse>> Handle(DeleteRangeOfferCommand? command, CancellationToken cancellationToken)
    {
        await Task.WhenAll();

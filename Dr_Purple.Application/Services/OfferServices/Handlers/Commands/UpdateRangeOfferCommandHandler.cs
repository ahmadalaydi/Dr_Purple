...
public class UpdateRangeOfferCommandHandler : BaseOfferCommandHandler,
    IRequestHandler<UpdateRangeOfferCommand, IDataResult<OfferResponse>>
{
    public UpdateRangeOfferCommandHandler(IServiceProvider provider) : base(provider) { }

    public async Task<IDataResult<OfferResponse>> Handle(UpdateRangeOfferCommand? command, CancellationToken cancellationToken)
    {
        await Task.WhenAll();

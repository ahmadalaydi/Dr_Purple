...
public class UpdateWareHouseCommandHandler : BaseWareHouseCommandHandler,
    IRequestHandler<UpdateWareHouseCommand, IDataResult<WareHouseResponse>>
{
    public UpdateWareHouseCommandHandler(IServiceProvider provider) : base(provider) { }

    public async Task<IDataResult<WareHouseResponse>> Handle(UpdateWareHouseCommand? command, CancellationToken cancellationToken)
    {
        await Task.WhenAll();

...
public class CreateWareHouseCommandHandler : BaseWareHouseCommandHandler,
    IRequestHandler<CreateWareHouseCommand, IDataResult<WareHouseResponse>>
{
    public CreateWareHouseCommandHandler(IServiceProvider provider) : base(provider) { }

    public async Task<IDataResult<WareHouseResponse>> Handle(CreateWareHouseCommand? command, CancellationToken cancellationToken)
    {
        await Task.WhenAll();

...
public class DeleteWareHouseCommandHandler : BaseWareHouseCommandHandler,
    IRequestHandler<DeleteWareHouseCommand, IDataResult<WareHouseResponse>>
{
    public DeleteWareHouseCommandHandler(IServiceProvider provider) : base(provider) { }

    public async Task<IDataResult<WareHouseResponse>> Handle(DeleteWareHouseCommand? command, CancellationToken cancellationToken)
    {
        await Task.WhenAll();

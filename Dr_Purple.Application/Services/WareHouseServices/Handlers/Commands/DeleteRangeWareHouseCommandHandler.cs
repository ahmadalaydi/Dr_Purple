...
public class DeleteRangeWareHouseCommandHandler : BaseWareHouseCommandHandler,
    IRequestHandler<DeleteRangeWareHouseCommand, IDataResult<WareHouseResponse>>
{
    public DeleteRangeWareHouseCommandHandler(IServiceProvider provider) : base(provider) { }

    public async Task<IDataResult<WareHouseResponse>> Handle(DeleteRangeWareHouseCommand? command, CancellationToken cancellationToken)
    {
        await Task.WhenAll();

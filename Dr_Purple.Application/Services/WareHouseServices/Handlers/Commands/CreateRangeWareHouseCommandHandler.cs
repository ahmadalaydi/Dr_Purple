...
public class CreateRangeWareHouseCommandHandler : BaseWareHouseCommandHandler,
    IRequestHandler<CreateRangeWareHouseCommand, IDataResult<WareHouseResponse>>
{
    public CreateRangeWareHouseCommandHandler(IServiceProvider provider) : base(provider) { }

    public async Task<IDataResult<WareHouseResponse>> Handle(CreateRangeWareHouseCommand? command, CancellationToken cancellationToken)
    {
        await Task.WhenAll();

...
public class UpdateRangeAttendCommandHandler : BaseAttendCommandHandler,
    IRequestHandler<UpdateRangeAttendCommand, IDataResult<AttendResponse>>
{
    public UpdateRangeAttendCommandHandler(IServiceProvider provider) : base(provider) { }

    public async Task<IDataResult<AttendResponse>> Handle(UpdateRangeAttendCommand? command, CancellationToken cancellationToken)
    {
        await Task.WhenAll();

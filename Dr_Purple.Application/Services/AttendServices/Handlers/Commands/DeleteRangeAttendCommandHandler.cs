...
public class DeleteRangeAttendCommandHandler : BaseAttendCommandHandler,
    IRequestHandler<DeleteRangeAttendCommand, IDataResult<AttendResponse>>
{
    public DeleteRangeAttendCommandHandler(IServiceProvider provider) : base(provider) { }

    public async Task<IDataResult<AttendResponse>> Handle(DeleteRangeAttendCommand? command, CancellationToken cancellationToken)
    {
        await Task.WhenAll();

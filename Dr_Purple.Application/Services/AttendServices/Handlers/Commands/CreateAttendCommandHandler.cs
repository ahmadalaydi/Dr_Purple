...
public class CreateAttendCommandHandler : BaseAttendCommandHandler,
    IRequestHandler<CreateAttendCommand, IDataResult<AttendResponse>>
{
    public CreateAttendCommandHandler(IServiceProvider provider) : base(provider) { }

    public async Task<IDataResult<AttendResponse>> Handle(CreateAttendCommand? command, CancellationToken cancellationToken)
    {
        await Task.WhenAll();

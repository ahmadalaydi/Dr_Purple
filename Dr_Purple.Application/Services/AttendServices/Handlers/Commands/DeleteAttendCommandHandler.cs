...
public class DeleteAttendCommandHandler : BaseAttendCommandHandler,
    IRequestHandler<DeleteAttendCommand, IDataResult<AttendResponse>>
{
    public DeleteAttendCommandHandler(IServiceProvider provider) : base(provider) { }

    public async Task<IDataResult<AttendResponse>> Handle(DeleteAttendCommand? command, CancellationToken cancellationToken)
    {
        await Task.WhenAll();

...
public class UpdateAttendCommandHandler : BaseAttendCommandHandler,
    IRequestHandler<UpdateAttendCommand, IDataResult<AttendResponse>>
{
    public UpdateAttendCommandHandler(IServiceProvider provider) : base(provider) { }

    public async Task<IDataResult<AttendResponse>> Handle(UpdateAttendCommand? command, CancellationToken cancellationToken)
    {
        await Task.WhenAll();

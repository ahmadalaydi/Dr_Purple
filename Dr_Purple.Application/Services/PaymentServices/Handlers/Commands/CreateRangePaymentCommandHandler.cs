...
public class CreateRangePaymentCommandHandler : BasePaymentCommandHandler,
    IRequestHandler<CreateRangePaymentCommand, IDataResult<PaymentResponse>>
{
    public CreateRangePaymentCommandHandler(IServiceProvider provider) : base(provider) { }

    public async Task<IDataResult<PaymentResponse>> Handle(CreateRangePaymentCommand? command, CancellationToken cancellationToken)
    {
        await Task.WhenAll();

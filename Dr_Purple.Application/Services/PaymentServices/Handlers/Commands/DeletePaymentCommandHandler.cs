...
public class DeletePaymentCommandHandler : BasePaymentCommandHandler,
    IRequestHandler<DeletePaymentCommand, IDataResult<PaymentResponse>>
{
    public DeletePaymentCommandHandler(IServiceProvider provider) : base(provider) { }

    public async Task<IDataResult<PaymentResponse>> Handle(DeletePaymentCommand? command, CancellationToken cancellationToken)
    {
        await Task.WhenAll();

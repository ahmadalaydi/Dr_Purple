...
public class DeleteRangePaymentCommandHandler : BasePaymentCommandHandler,
    IRequestHandler<DeleteRangePaymentCommand, IDataResult<PaymentResponse>>
{
    public DeleteRangePaymentCommandHandler(IServiceProvider provider) : base(provider) { }

    public async Task<IDataResult<PaymentResponse>> Handle(DeleteRangePaymentCommand? command, CancellationToken cancellationToken)
    {
        await Task.WhenAll();

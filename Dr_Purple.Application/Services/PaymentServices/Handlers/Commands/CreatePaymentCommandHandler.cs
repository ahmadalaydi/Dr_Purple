...
public class CreatePaymentCommandHandler : BasePaymentCommandHandler,
    IRequestHandler<CreatePaymentCommand, IDataResult<PaymentResponse>>
{
    public CreatePaymentCommandHandler(IServiceProvider provider) : base(provider) { }

    public async Task<IDataResult<PaymentResponse>> Handle(CreatePaymentCommand? command, CancellationToken cancellationToken)
    {
        await Task.WhenAll();

...
public class UpdateRangePaymentCommandHandler : BasePaymentCommandHandler,
    IRequestHandler<UpdateRangePaymentCommand, IDataResult<PaymentResponse>>
{
    public UpdateRangePaymentCommandHandler(IServiceProvider provider) : base(provider) { }

    public async Task<IDataResult<PaymentResponse>> Handle(UpdateRangePaymentCommand? command, CancellationToken cancellationToken)
    {
        await Task.WhenAll();

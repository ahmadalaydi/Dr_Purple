...
public class UpdateAppointmentCommandHandler : BaseAppointmentCommandHandler,
    IRequestHandler<UpdateAppointmentCommand, IDataResult<AppointmentResponse>>
{
    public UpdateAppointmentCommandHandler(IServiceProvider provider) : base(provider) { }

    public async Task<IDataResult<AppointmentResponse>> Handle(UpdateAppointmentCommand? command, CancellationToken cancellationToken)
    {
        await Task.WhenAll();

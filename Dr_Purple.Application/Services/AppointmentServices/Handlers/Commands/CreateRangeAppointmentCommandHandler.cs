...
public class CreateRangeAppointmentCommandHandler : BaseAppointmentCommandHandler,
    IRequestHandler<CreateRangeAppointmentCommand, IDataResult<AppointmentResponse>>
{
    public CreateRangeAppointmentCommandHandler(IServiceProvider provider) : base(provider) { }

    public async Task<IDataResult<AppointmentResponse>> Handle(CreateRangeAppointmentCommand? command, CancellationToken cancellationToken)
    {
        await Task.WhenAll();

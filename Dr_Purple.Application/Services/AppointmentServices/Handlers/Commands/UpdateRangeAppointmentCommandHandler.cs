...
public class UpdateRangeAppointmentCommandHandler : BaseAppointmentCommandHandler,
    IRequestHandler<UpdateRangeAppointmentCommand, IDataResult<AppointmentResponse>>
{
    public UpdateRangeAppointmentCommandHandler(IServiceProvider provider) : base(provider) { }

    public async Task<IDataResult<AppointmentResponse>> Handle(UpdateRangeAppointmentCommand? command, CancellationToken cancellationToken)
    {
        await Task.WhenAll();

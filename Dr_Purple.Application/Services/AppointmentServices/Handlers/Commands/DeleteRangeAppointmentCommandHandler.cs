...
public class DeleteRangeAppointmentCommandHandler : BaseAppointmentCommandHandler,
    IRequestHandler<DeleteRangeAppointmentCommand, IDataResult<AppointmentResponse>>
{
    public DeleteRangeAppointmentCommandHandler(IServiceProvider provider) : base(provider) { }

    public async Task<IDataResult<AppointmentResponse>> Handle(DeleteRangeAppointmentCommand? command, CancellationToken cancellationToken)
    {
        await Task.WhenAll();

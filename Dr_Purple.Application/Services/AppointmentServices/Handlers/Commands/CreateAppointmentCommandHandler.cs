...
public class CreateAppointmentCommandHandler : BaseAppointmentCommandHandler,
    IRequestHandler<CreateAppointmentCommand, IDataResult<AppointmentResponse>>
{
    public CreateAppointmentCommandHandler(IServiceProvider provider) : base(provider) { }

    public async Task<IDataResult<AppointmentResponse>> Handle(CreateAppointmentCommand? command, CancellationToken cancellationToken)
    {
        await Task.WhenAll();

...
public class UpdateAdditionCommandHandler : BaseAdditionCommandHandler,
    IRequestHandler<UpdateAdditionCommand, IDataResult<AdditionResponse>>
{
    public UpdateAdditionCommandHandler(IServiceProvider provider) : base(provider) { }

    public async Task<IDataResult<AdditionResponse>> Handle(UpdateAdditionCommand? command, CancellationToken cancellationToken)
    {
        await Task.WhenAll();

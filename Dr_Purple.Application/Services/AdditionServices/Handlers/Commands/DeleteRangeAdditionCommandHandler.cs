...
public class DeleteRangeAdditionCommandHandler : BaseAdditionCommandHandler,
    IRequestHandler<DeleteRangeAdditionCommand, IDataResult<AdditionResponse>>
{
    public DeleteRangeAdditionCommandHandler(IServiceProvider provider) : base(provider) { }

    public async Task<IDataResult<AdditionResponse>> Handle(DeleteRangeAdditionCommand? command, CancellationToken cancellationToken)
    {
        await Task.WhenAll();

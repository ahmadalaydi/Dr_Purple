...
public class CreateRangeAdditionCommandHandler : BaseAdditionCommandHandler,
    IRequestHandler<CreateRangeAdditionCommand, IDataResult<AdditionResponse>>
{
    public CreateRangeAdditionCommandHandler(IServiceProvider provider) : base(provider) { }

    public async Task<IDataResult<AdditionResponse>> Handle(CreateRangeAdditionCommand? command, CancellationToken cancellationToken)
    {
        await Task.WhenAll();

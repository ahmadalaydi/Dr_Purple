...
public class UpdateRangeAdditionCommandHandler : BaseAdditionCommandHandler,
    IRequestHandler<UpdateRangeAdditionCommand, IDataResult<AdditionResponse>>
{
    public UpdateRangeAdditionCommandHandler(IServiceProvider provider) : base(provider) { }

    public async Task<IDataResult<AdditionResponse>> Handle(UpdateRangeAdditionCommand? command, CancellationToken cancellationToken)
    {
        await Task.WhenAll();

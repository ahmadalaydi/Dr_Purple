...
public class UpdateRangeServiceCommandHandler : BaseServiceCommandHandler,
    IRequestHandler<UpdateRangeServiceCommand, IDataResult<ServiceResponse>>
{
    public UpdateRangeServiceCommandHandler(IServiceProvider provider) : base(provider) { }

    public async Task<IDataResult<ServiceResponse>> Handle(UpdateRangeServiceCommand? command, CancellationToken cancellationToken)
    {
        await Task.WhenAll();

...
public class CreateRangeServiceCommandHandler : BaseServiceCommandHandler,
    IRequestHandler<CreateRangeServiceCommand, IDataResult<ServiceResponse>>
{
    public CreateRangeServiceCommandHandler(IServiceProvider provider) : base(provider) { }

    public async Task<IDataResult<ServiceResponse>> Handle(CreateRangeServiceCommand? command, CancellationToken cancellationToken)
    {
        await Task.WhenAll();

...
public class UpdateServiceCommandHandler : BaseServiceCommandHandler,
    IRequestHandler<UpdateServiceCommand, IDataResult<ServiceResponse>>
{
    public UpdateServiceCommandHandler(IServiceProvider provider) : base(provider) { }

    public async Task<IDataResult<ServiceResponse>> Handle(UpdateServiceCommand? command, CancellationToken cancellationToken)
    {
        await Task.WhenAll();

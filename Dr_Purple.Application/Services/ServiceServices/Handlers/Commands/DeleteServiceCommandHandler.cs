...
public class DeleteServiceCommandHandler : BaseServiceCommandHandler,
    IRequestHandler<DeleteServiceCommand, IDataResult<ServiceResponse>>
{
    public DeleteServiceCommandHandler(IServiceProvider provider) : base(provider) { }

    public async Task<IDataResult<ServiceResponse>> Handle(DeleteServiceCommand? command, CancellationToken cancellationToken)
    {
        await Task.WhenAll();

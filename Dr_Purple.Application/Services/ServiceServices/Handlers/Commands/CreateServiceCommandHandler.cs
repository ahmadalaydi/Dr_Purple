...
public class CreateServiceCommandHandler : BaseServiceCommandHandler,
    IRequestHandler<CreateServiceCommand, IDataResult<ServiceResponse>>
{
    public CreateServiceCommandHandler(IServiceProvider provider) : base(provider) { }

    public async Task<IDataResult<ServiceResponse>> Handle(CreateServiceCommand? command, CancellationToken cancellationToken)
    {
        await Task.WhenAll();

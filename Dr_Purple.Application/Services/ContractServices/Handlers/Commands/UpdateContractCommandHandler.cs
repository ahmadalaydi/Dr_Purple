...
public class UpdateContractCommandHandler : BaseContractCommandHandler,
    IRequestHandler<UpdateContractCommand, IDataResult<ContractResponse>>
{
    public UpdateContractCommandHandler(IServiceProvider provider) : base(provider) { }

    public async Task<IDataResult<ContractResponse>> Handle(UpdateContractCommand? command, CancellationToken cancellationToken)
    {
        await Task.WhenAll();

...
public class UpdateRangeContractCommandHandler : BaseContractCommandHandler,
    IRequestHandler<UpdateRangeContractCommand, IDataResult<ContractResponse>>
{
    public UpdateRangeContractCommandHandler(IServiceProvider provider) : base(provider) { }

    public async Task<IDataResult<ContractResponse>> Handle(UpdateRangeContractCommand? command, CancellationToken cancellationToken)
    {
        await Task.WhenAll();

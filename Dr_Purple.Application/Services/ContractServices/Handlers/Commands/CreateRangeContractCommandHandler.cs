...
public class CreateRangeContractCommandHandler : BaseContractCommandHandler,
    IRequestHandler<CreateRangeContractCommand, IDataResult<ContractResponse>>
{
    public CreateRangeContractCommandHandler(IServiceProvider provider) : base(provider) { }

    public async Task<IDataResult<ContractResponse>> Handle(CreateRangeContractCommand? command, CancellationToken cancellationToken)
    {
        await Task.WhenAll();

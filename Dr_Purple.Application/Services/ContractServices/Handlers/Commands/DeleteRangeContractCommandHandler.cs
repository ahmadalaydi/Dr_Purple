...
public class DeleteRangeContractCommandHandler : BaseContractCommandHandler,
    IRequestHandler<DeleteRangeContractCommand, IDataResult<ContractResponse>>
{
    public DeleteRangeContractCommandHandler(IServiceProvider provider) : base(provider) { }

    public async Task<IDataResult<ContractResponse>> Handle(DeleteRangeContractCommand? command, CancellationToken cancellationToken)
    {
        await Task.WhenAll();

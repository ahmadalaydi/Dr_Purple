...
public class CreateContractCommandHandler : BaseContractCommandHandler,
    IRequestHandler<CreateContractCommand, IDataResult<ContractResponse>>
{
    public CreateContractCommandHandler(IServiceProvider provider) : base(provider) { }

    public async Task<IDataResult<ContractResponse>> Handle(CreateContractCommand? command, CancellationToken cancellationToken)
    {
        await Task.WhenAll();

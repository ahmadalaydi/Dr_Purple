...
public class DeleteLeaveCommandHandler : BaseLeaveCommandHandler,
    IRequestHandler<DeleteLeaveCommand, IDataResult<LeaveResponse>>
{
    public DeleteLeaveCommandHandler(IServiceProvider provider) : base(provider) { }

    public async Task<IDataResult<LeaveResponse>> Handle(DeleteLeaveCommand? command, CancellationToken cancellationToken)
    {
        await Task.WhenAll();

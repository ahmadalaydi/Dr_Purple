...
public class DeleteRangeLeaveCommandHandler : BaseLeaveCommandHandler,
    IRequestHandler<DeleteRangeLeaveCommand, IDataResult<LeaveResponse>>
{
    public DeleteRangeLeaveCommandHandler(IServiceProvider provider) : base(provider) { }

    public async Task<IDataResult<LeaveResponse>> Handle(DeleteRangeLeaveCommand? command, CancellationToken cancellationToken)
    {
        await Task.WhenAll();

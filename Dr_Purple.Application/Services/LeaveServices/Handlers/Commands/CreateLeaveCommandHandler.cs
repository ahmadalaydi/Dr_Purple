...
public class CreateLeaveCommandHandler : BaseLeaveCommandHandler,
    IRequestHandler<CreateLeaveCommand, IDataResult<LeaveResponse>>
{
    public CreateLeaveCommandHandler(IServiceProvider provider) : base(provider) { }

    public async Task<IDataResult<LeaveResponse>> Handle(CreateLeaveCommand? command, CancellationToken cancellationToken)
    {
        await Task.WhenAll();

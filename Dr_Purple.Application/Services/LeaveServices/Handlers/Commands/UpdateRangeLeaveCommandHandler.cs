...
public class UpdateRangeLeaveCommandHandler : BaseLeaveCommandHandler,
    IRequestHandler<UpdateRangeLeaveCommand, IDataResult<LeaveResponse>>
{
    public UpdateRangeLeaveCommandHandler(IServiceProvider provider) : base(provider) { }

    public async Task<IDataResult<LeaveResponse>> Handle(UpdateRangeLeaveCommand? command, CancellationToken cancellationToken)
    {
        await Task.WhenAll();

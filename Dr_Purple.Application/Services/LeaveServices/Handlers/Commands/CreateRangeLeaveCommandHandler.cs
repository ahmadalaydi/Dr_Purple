...
public class CreateRangeLeaveCommandHandler : BaseLeaveCommandHandler,
    IRequestHandler<CreateRangeLeaveCommand, IDataResult<LeaveResponse>>
{
    public CreateRangeLeaveCommandHandler(IServiceProvider provider) : base(provider) { }

    public async Task<IDataResult<LeaveResponse>> Handle(CreateRangeLeaveCommand? command, CancellationToken cancellationToken)
    {
        await Task.WhenAll();

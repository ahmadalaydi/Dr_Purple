...
public class UpdateRangeUnitCommandHandler : BaseUnitCommandHandler,
    IRequestHandler<UpdateRangeUnitCommand, IDataResult<UnitResponse>>
{
    public UpdateRangeUnitCommandHandler(IServiceProvider provider) : base(provider) { }

    public async Task<IDataResult<UnitResponse>> Handle(UpdateRangeUnitCommand? command, CancellationToken cancellationToken)
    {
        await Task.WhenAll();

...
public class UpdateUnitCommandHandler : BaseUnitCommandHandler,
    IRequestHandler<UpdateUnitCommand, IDataResult<UnitResponse>>
{
    public UpdateUnitCommandHandler(IServiceProvider provider) : base(provider) { }

    public async Task<IDataResult<UnitResponse>> Handle(UpdateUnitCommand? command, CancellationToken cancellationToken)
    {
        await Task.WhenAll();

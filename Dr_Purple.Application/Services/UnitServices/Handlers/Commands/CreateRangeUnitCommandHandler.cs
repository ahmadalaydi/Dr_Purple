...
public class CreateRangeUnitCommandHandler : BaseUnitCommandHandler,
    IRequestHandler<CreateRangeUnitCommand, IDataResult<UnitResponse>>
{
    public CreateRangeUnitCommandHandler(IServiceProvider provider) : base(provider) { }

    public async Task<IDataResult<UnitResponse>> Handle(CreateRangeUnitCommand? command, CancellationToken cancellationToken)
    {
        await Task.WhenAll();

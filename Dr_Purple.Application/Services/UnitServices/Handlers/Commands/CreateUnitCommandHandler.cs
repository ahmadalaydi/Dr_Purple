...
public class CreateUnitCommandHandler : BaseUnitCommandHandler,
    IRequestHandler<CreateUnitCommand, IDataResult<UnitResponse>>
{
    public CreateUnitCommandHandler(IServiceProvider provider) : base(provider) { }

    public async Task<IDataResult<UnitResponse>> Handle(CreateUnitCommand? command, CancellationToken cancellationToken)
    {
        await Task.WhenAll();

...
public class DeleteUnitCommandHandler : BaseUnitCommandHandler,
    IRequestHandler<DeleteUnitCommand, IDataResult<UnitResponse>>
{
    public DeleteUnitCommandHandler(IServiceProvider provider) : base(provider) { }

    public async Task<IDataResult<UnitResponse>> Handle(DeleteUnitCommand? command, CancellationToken cancellationToken)
    {
        await Task.WhenAll();

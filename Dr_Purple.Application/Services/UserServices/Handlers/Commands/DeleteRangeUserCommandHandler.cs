...
public class DeleteRangeUserCommandHandler : BaseUserCommandHandler,
    IRequestHandler<DeleteRangeUserCommand, IDataResult<UserResponse>>
{
    public DeleteRangeUserCommandHandler(IServiceProvider provider) : base(provider) { }

    public async Task<IDataResult<UserResponse>> Handle(DeleteRangeUserCommand? command, CancellationToken cancellationToken)
    {
        await Task.WhenAll();

...
public class CreateUserCommandHandler : BaseUserCommandHandler,
    IRequestHandler<CreateUserCommand, IDataResult<UserResponse>>
{
    public CreateUserCommandHandler(IServiceProvider provider) : base(provider) { }

    public async Task<IDataResult<UserResponse>> Handle(CreateUserCommand? command, CancellationToken cancellationToken)
    {
        await Task.WhenAll();

...
public class UpdateRangeUserCommandHandler : BaseUserCommandHandler,
    IRequestHandler<UpdateRangeUserCommand, IDataResult<UserResponse>>
{
    public UpdateRangeUserCommandHandler(IServiceProvider provider) : base(provider) { }

    public async Task<IDataResult<UserResponse>> Handle(UpdateRangeUserCommand? command, CancellationToken cancellationToken)
    {
        await Task.WhenAll();

...
public class CreateRangeUserCommandHandler : BaseUserCommandHandler,
    IRequestHandler<CreateRangeUserCommand, IDataResult<UserResponse>>
{
    public CreateRangeUserCommandHandler(IServiceProvider provider) : base(provider) { }

    public async Task<IDataResult<UserResponse>> Handle(CreateRangeUserCommand? command, CancellationToken cancellationToken)
    {
        await Task.WhenAll();

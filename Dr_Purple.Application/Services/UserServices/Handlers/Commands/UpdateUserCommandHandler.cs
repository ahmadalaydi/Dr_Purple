...
public class UpdateUserCommandHandler : BaseUserCommandHandler,
    IRequestHandler<UpdateUserCommand, IDataResult<UserResponse>>
{
    public UpdateUserCommandHandler(IServiceProvider provider) : base(provider) { }

    public async Task<IDataResult<UserResponse>> Handle(UpdateUserCommand? command, CancellationToken cancellationToken)
    {
        await Task.WhenAll();

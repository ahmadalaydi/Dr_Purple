...
public class DeleteSubDepartmentCommandHandler : BaseSubDepartmentCommandHandler,
    IRequestHandler<DeleteSubDepartmentCommand, IDataResult<SubDepartmentResponse>>
{
    public DeleteSubDepartmentCommandHandler(IServiceProvider provider) : base(provider) { }

    public async Task<IDataResult<SubDepartmentResponse>> Handle(DeleteSubDepartmentCommand? command, CancellationToken cancellationToken)
    {
        await Task.WhenAll();

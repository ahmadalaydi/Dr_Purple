...
public class DeleteRangeSubDepartmentCommandHandler : BaseSubDepartmentCommandHandler,
    IRequestHandler<DeleteRangeSubDepartmentCommand, IDataResult<SubDepartmentResponse>>
{
    public DeleteRangeSubDepartmentCommandHandler(IServiceProvider provider) : base(provider) { }

    public async Task<IDataResult<SubDepartmentResponse>> Handle(DeleteRangeSubDepartmentCommand? command, CancellationToken cancellationToken)
    {
        await Task.WhenAll();

...
public class CreateSubDepartmentCommandHandler : BaseSubDepartmentCommandHandler,
    IRequestHandler<CreateSubDepartmentCommand, IDataResult<SubDepartmentResponse>>
{
    public CreateSubDepartmentCommandHandler(IServiceProvider provider) : base(provider) { }

    public async Task<IDataResult<SubDepartmentResponse>> Handle(CreateSubDepartmentCommand? command, CancellationToken cancellationToken)
    {
        await Task.WhenAll();

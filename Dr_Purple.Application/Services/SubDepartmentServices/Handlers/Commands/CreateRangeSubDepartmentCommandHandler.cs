...
public class CreateRangeSubDepartmentCommandHandler : BaseSubDepartmentCommandHandler,
    IRequestHandler<CreateRangeSubDepartmentCommand, IDataResult<SubDepartmentResponse>>
{
    public CreateRangeSubDepartmentCommandHandler(IServiceProvider provider) : base(provider) { }

    public async Task<IDataResult<SubDepartmentResponse>> Handle(CreateRangeSubDepartmentCommand? command, CancellationToken cancellationToken)
    {
        await Task.WhenAll();

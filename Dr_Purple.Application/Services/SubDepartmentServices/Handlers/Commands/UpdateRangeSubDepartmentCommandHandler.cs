...
public class UpdateRangeSubDepartmentCommandHandler : BaseSubDepartmentCommandHandler,
    IRequestHandler<UpdateRangeSubDepartmentCommand, IDataResult<SubDepartmentResponse>>
{
    public UpdateRangeSubDepartmentCommandHandler(IServiceProvider provider) : base(provider) { }

    public async Task<IDataResult<SubDepartmentResponse>> Handle(UpdateRangeSubDepartmentCommand? command, CancellationToken cancellationToken)
    {
        await Task.WhenAll();
